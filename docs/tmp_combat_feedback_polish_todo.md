...
- [x] Combo popup obstruction
  ...
  - Implementation: moved/clamped combo popup placement off-cluster and added dark translucent panel + stacked content in `scripts/scenes/combat.gd:2294-2367`.
  - Effort:
    1. Created temporary tracker before runtime edits and recorded the original source refs for this issue.
    2. Added off-cluster placement, board clamping, and readable backplate in `scripts/scenes/combat.gd:2294-2367`.
    3. Fixed MCP strict-warning issues in combo popup helper locals in `scripts/scenes/combat.gd:2300-2303,2312-2315`.
...
- [x] Enemy impact square/artifact
  ...
  - Effort:
    ...
    2. Centralized replay impact emission under `_replay_turn_resolution_from_log()` in `scripts/scenes/combat.gd:1618-1658`.
    3. Fixed strict-warning issues in replay impact helper code in `scripts/scenes/combat.gd:2124,2176-2179,2225`.
  - Validation notes: pending runtime screenshot pass.
...
- [x] Detached/clipped mastery beam
  ...
  - Implementation: source now resolves via `_elemental_mastery_cards` (`scripts/scenes/combat.gd:76,2188-2197`), beam trigger path is centralized in `_replay_turn_resolution_from_log()` (`scripts/scenes/combat.gd:1618-1658`), and helper remains backward-compatible (`scripts/scenes/combat.gd:2230-2259`).
  - Effort:
    1. Aligned beam source ownership to active mastery cards via `_elemental_mastery_cards` in `scripts/scenes/combat.gd:76,2188-2197`.
    2. Centralized mastery beam replay path in `_replay_turn_resolution_from_log()` at `scripts/scenes/combat.gd:1618-1658`.
    3. Fixed parser typing regression in beam math locals with explicit `Vector2`/`float` types in `scripts/scenes/combat.gd:2256-2263`.
    4. Restored Elemental Mastery panel layout constants and `_apply_combat_mastery_panel_layout()` in `scripts/scenes/combat.gd:77-78,138-149,2454,2478,2561-2586`.
    5. Fixed runtime crash from invalid `Control.to_local()` calls by converting source/target globals with `_vfx_layer.get_global_transform_with_canvas().affine_inverse()` in `scripts/scenes/combat.gd:2264-2266`.
    6. Rebalanced the active mastery card row to fit six cards inside the panel and moved the title above the cards in `scripts/scenes/combat.gd:138-141` and `scripts/ui/player_loadout_hud.gd:15-17,189-214`.
    7. Wired `turn_log` values into transient card feedback labels and allowed elemental beams to fire from per-element values even when enemy block absorbs all HP damage in `scripts/scenes/combat.gd:1626-1708`.
  - Validation notes: pending runtime screenshot pass.
...
- [x] Duplicate locked timer text
  ...
  - Current source references: `scripts/scenes/combat.gd:1486` (state set), `scripts/scenes/combat.gd:1487` (state hidden).
  - Implementation: set `_timer_label.text = "LOCK"` and `_timer_state_label.text = ""` in `scripts/scenes/combat.gd:1486-1487`.
  - Effort:
    1. Changed locked timer state to one visible label in `scripts/scenes/combat.gd:1486-1487`.
    2. Kept the hidden secondary timer-state label available for non-lock states without adding new UI nodes.
    3. Suppressed legacy bottom mastery strip while validating the same combat layout pass in `scripts/scenes/combat.gd:2105-2110,2111-2117,2544-2549`.
...
- [x] Elemental mastery card sizing + replay feedback visibility
  ...
  - Implementation: updated `scripts/ui/player_loadout_hud.gd` card geometry constants + label positions, then mapped feedback values in
    `scripts/scenes/combat.gd::_replay_turn_resolution_from_log()` and `_play_combat_mastery_feedback_from_turn_log()` with short stagger and clear.
  - Effort:
    1. Added small timer-staggered feedback sequence so cards reveal in left-to-right-ish order (`COMBAT_MASTERY_FEEDBACK_STAGGER_SECONDS`) and clear afterwards in `scripts/scenes/combat.gd:1674-1708`.
    2. Triggered elemental beams from per-element values even when `enemy_damage_taken == 0` by using per-element checks in `scripts/scenes/combat.gd:1645-1658`.
    3. Kept player panel anchor region unchanged and reflowed only the board/mastery split in `scripts/scenes/combat.gd:138-142`.
    4. Corrected card overflow from `176px` to `164px` cards with `8px` gaps in `scripts/ui/player_loadout_hud.gd:15-17`.
    5. Corrected title/card and label/level/feedback overlaps in `scripts/scenes/combat.gd:140-141` and `scripts/ui/player_loadout_hud.gd:189-214`.
  - Validation notes:
...
- 2026-04-30: MCP parse failure observed (inference errors): `_spawn_mastery_beam` locals `source_local`, `target_local`, `delta`, `distance` at
  `scripts/scenes/combat.gd:2260-2263`.
...
- 2026-04-30: MCP parse failure observed (inference errors): `_spawn_combo_floating_text` locals `cell_span` and `blocked_radius` at
  `scripts/scenes/combat.gd:2320-2321`.
...
- 2026-04-30: MCP strict-warning failure observed in new helper code: unused parameter, enum/integer-narrowing, and float-bound conversion warnings in
  `scripts/scenes/combat.gd:2124,2176-2179,2225,2300-2315`.
- 2026-04-30: MCP strict-warning fixes applied with explicit typing and float helpers in helper code:
  `scripts/scenes/combat.gd:2124`, `scripts/scenes/combat.gd:2176-2179`, `scripts/scenes/combat.gd:2225`,
  `scripts/scenes/combat.gd:2300-2303`, `scripts/scenes/combat.gd:2312-2315`.
...
- 2026-04-30: MCP regression fixed by suppressing legacy strip display and population in `combat_player_controller.gd`:
  `scripts/scenes/combat.gd:2105-2110,2111-2117,2544-2549`.
...
- 2026-04-30: Godot MCP load/instantiate probe passed with `ResourceLoader.CACHE_MODE_IGNORE` for changed scripts and
  `res://scenes/combat.tscn`: controller, board, registry, scene, and instance all loaded.
...
- 2026-04-30: Overflow/overlap fixed with `164x186` cards, `8px` gaps, cards rect `Vector2(10,40)/Vector2(1028,186)`,
  and non-overlapping card internals in `scripts/scenes/combat.gd:138-141` and
