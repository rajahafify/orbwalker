...
- [x] Combo count and turn result feedback are readable.
  ...
  - 2026-05-04: CFR-08 implementation adds presentation-only projected enemy-attack and enemy-block previews before the player moves. `CombatPlayerController` computes the HP-loss preview from the current intent attack value plus visible player HP/armor, while `PlayerLoadoutHud` renders a red-to-empty-to-red blinking HP danger segment for unblocked/partially blocked HP loss and an armor-risk pulse for fully blocked attacks. The transparent phase reveals an empty HP backing instead of the normal HP fill. The controller also computes an enemy intent preview dictionary with an `entries` array for future multi-intent presentation; today's supported `block` entry renders as a blinking silver overshield strip above the enemy HP bar with width `intent.block / enemy.max_hp`. Hovering either preview only scales/blinks the existing enemy intent display until hover exits and updates status text; combat math, enemy intent values, resolver behavior, RunState routing, enemy attack resolution, replay timing, board resolve order, and `combat_speed` were not intentionally changed. `git diff --check` passed. Godot MCP reached Godot 4.6.2, loaded both edited scripts, instantiated `res://scenes/combat/combat_player.tscn`, confirmed required HP/intent/HUD nodes, probed projected-damage math for unblocked, partially blocked, fully blocked, overkill-clamped, and no-attack cases, probed HUD overlay visibility/sizing/mouse-filter behavior, confirmed the hover enter/exit signal wiring and full-red danger fill over empty backing, probed enemy block-preview sizing/hide behavior, and launched `play_scene current`. The final `get_godot_errors` read still reports the two known stale CFR-02 enum reload diagnostics and no new CFR-08 runtime errors. Manual visual QA remains needed for real enemy intents and hover readability.
