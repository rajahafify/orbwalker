...
- Focused Godot MCP encounter-stat probe confirmed L1/L2 stat changes and L3 preservation: L1F1 is now `38 HP` with `0/5/6` attacks and `8/6/0` block, L1F2 `41 HP` with `0/4/5` attacks and `12/9/0` block, L1 boss `85 HP` with `0/9/10` attacks and `20/12/0` block, L2F1 `85 HP` with `16/14/18` attacks and `0/3/0` block, L2F2 `88 HP` with `18/16/15` attacks and `0/0/2` block, L2 boss `158 HP` with `26/24/20` attacks and `0/0/4` block, L3F1 remains `112 HP` with `18/12/17` attacks, and L3 boss remains `176 HP` with `0/24/16` attacks. A mastery-0 basic 3-orb elemental match with `1` combo deals `3` damage before enemy block, so the level 1 values are deliberately forgiving for that low-skill floor.
- Player HP rules, combat math, resolver behavior, shop transaction semantics, RunState routing, Run Log behavior, combat presentation timing, and M10-04 fixed fight-gold reward/popup behavior were not changed. Focused guard probes confirmed level fight rewards remain `10/12/14`, first fight victory with no matched Gold still grants `10` base gold, and level 3 stat signatures stay unchanged. One probe using normal `load(...)` saw stale cached script values; rerunning with `ResourceLoader.CACHE_MODE_IGNORE` confirmed the current identity values.
