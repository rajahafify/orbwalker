...
- Validation: No runtime validation required for this inventory pass. Evidence was gathered by inspecting `scripts/scenes/combat.gd`, `scripts/combat/combat_resolve_presenter.gd`, `scripts/combat/combat_vfx_presenter.gd`, `scripts/combat/combat_state_machine.gd`, `scripts/combat/combat_turn_log_presenter.gd`, `scripts/combat/combat_hud_presenter.gd`, `scripts/ui/player_loadout_hud.gd`, and `scenes/combat.tscn`.
...
- Add result-number presentation near the existing replay impact path, likely around `_replay_turn_resolution_from_log()` and `CombatVfxPresenter`, rather than changing resolver math or match resolution.
...
- Progress: Completed on `codex/cfr-04-mastery-effect-readability`: combat mastery cards now add fixed-size activation glow/frame layers behind the existing card text, scale activation intensity from the pooled contribution value, and pulse briefly when `set_combat_mastery_feedback(...)` raises a card's visible contribution. Mastery beams still originate from the active card/icon, and `CombatVfxPresenter` now adds a stronger source pulse when the beam fires so the player can read the card as the source. A follow-up after screenshot QA carries active pooled feedback totals through HUD rebuilds, so card lights do not clear as a group during staged HUD updates and still release through the per-card replay timing. Mastery Effect SFX now plays at each replay impact/result moment for damage, heal, armor, and gold, reusing existing placeholder `hit`, `heal`, `armor`, and `gold` sounds without adding source-launch audio. This is presentation-only; combat math, match resolver behavior, RunState routing, turn-log payloads, result label text, enemy HP/block stepping, board resolve order, and `combat_speed` modes were not changed.
...
- Progress: Completed on `codex/cfr-05-vfx-tier-hooks`: `CombatVfxPresenter` now owns temporary per-kind thresholds for Fire, Ice, Earth, Heart, Armor, and Gold replay impacts, returns four positive VFX tiers, and scales existing/fallback impact size, lifetime, alpha, brightness, result-label font size, label outline, and label container size by tier. `CombatPlayerController` passes existing `turn_log` replay values into the impact and positive result-label helpers, without changing combat math, match resolver behavior, RunState routing, board resolve order, result label text, SFX timing, staged HUD stepping, mastery feedback release, or `combat_speed` modes.
- Blockers: None.
...
- Validation: `git diff --check` passed. Godot MCP reached Godot 4.6.2, opened `combat_vfx_presenter.gd`, `combat_player_controller.gd`, and `visual_registry.gd`, confirmed four-tier threshold boundaries for Fire/Ice/Earth/Heart/Armor/Gold with a focused `CombatVfxPresenter` helper probe, instantiated `res://scenes/combat.tscn` and confirmed `VfxLayer`, `ElementalMasteryCards`, enemy portrait, player portrait, and board surface exist, spawned tiered replay impacts for all six kinds through the existing `VfxLayer` fallback path, confirmed lowered threshold mapping, confirmed tiered positive label font sizing, and launched `play_scene current`. User visual QA passed on 2026-05-04 after increasing tier scales to `1.0`, `1.5`, `2.0`, `3.0`, lowering early-run thresholds, and scaling positive result labels by tier. The first helper probe hit a stale cached script and returned a missing-method error before rerunning with `ResourceLoader.CACHE_MODE_IGNORE`; the final `get_godot_errors` read still reports the two known stale CFR-02 enum reload diagnostics.
...
## CFR-09: Pre-Drag Mastery Hover Readability

- Status: `done`
- Owner/scope: Add pre-drag hover affordances that connect board orb types to the matching Elemental Mastery card, and add inspectable mastery-card details for damage/resource contribution and build modifiers.
- Progress: Runtime implementation added on 2026-05-05. `BoardView` now exposes presentation-only orb lookup helpers for board-local pointer positions, `CombatPlayerController` forwards pre-drag board-orb hover to the shared HUD and clears hover state on drag start, non-player-input phases, turn preview refresh, board mouse exit, and scene exit, and `PlayerLoadoutHud` renders a separate hover-only mastery-card highlight plus compact mastery detail panel. Hovering a mastery card also lights equipment and relic tokens whose current modifier source affects that mastery/orb type. Hover visuals use dedicated hover nodes and do not reuse or clear active combat replay feedback. Remaining manual visual QA across all six orb/card types and overlap/readability is transferred to ITCH-01 and ITCH-08 in `docs/itch_readiness_tasks.md`.
- Blockers: None known. Implementation should start from the existing board hover/input path and `PlayerLoadoutHud` mastery-card rendering instead of changing combat resolution.
- Next action: Closed in this tracker. Public-readiness validation continues under ITCH-01/ITCH-08:
  - hovering a board orb highlights the matching Fire, Ice, Earth, Heart, Armor, or Gold mastery card;
  - starting a drag should clear or suppress the hover-only highlight so it does not conflict with active resolve feedback;
  - hovering an Elemental Mastery card opens a compact detail label/panel with more specific contribution information.
- Detail label content target:
  - current mastery level and base effect;
  - damage or resource value per matching orb, using the same source values the combat preview/turn resolution already uses where possible;
  - equipment and relic names that currently affect that mastery or orb type;
  - clear empty state such as `No equipment or relic modifiers` when nothing affects it.
- Acceptance:
  - Before drag, hovering a Fire/Ice/Earth/Heart/Armor/Gold orb visibly lights up the corresponding Elemental Mastery card only.
  - Moving off the orb, starting a drag, resolving a turn, or leaving combat clears the hover highlight without clearing active mastery feedback from combat replay.
  - Hovering an Elemental Mastery card shows a readable detail label/panel near the card or mastery rail.
  - The detail label includes Damage Per Orb or resource equivalent for that mastery, plus equipment/relic modifiers affecting that mastery when present.
  - The detail label hides when the card is no longer hovered/focused and does not permanently cover the board, enemy HP/intent, or player HUD.
  - Existing combat math, match resolver behavior, RunState routing, result-label text, staged HUD stepping, mastery replay feedback, and `combat_speed` modes are unchanged.
- Validation: Code-complete automated validation passed on 2026-05-05. `git diff --check` passed. Godot MCP reached Godot 4.6.2, loaded `board_view.gd`, `combat_player_controller.gd`, and `player_loadout_hud.gd`, loaded/instantiated `res://scenes/combat.tscn`, and confirmed `Board`, `ElementalMasteryCards`, `PlayerHudSection`, and `VfxLayer` exist. Focused probes confirmed board-local orb lookup maps a Fire cell to Fire and gaps to `-1`, Fire board-hover lights only the Fire mastery card, clearing hover removes the Fire hover frame, active Ice replay feedback remains visible while Fire hover is active, card hover shows/hides the detail bubble, detail content includes mastery level/base effect/per-orb value/modifier names, and the empty state reads `No equipment or relic modifiers`. A follow-up hover-source probe confirmed Fire card hover lights only matching equipment, Gold card hover lights only matching relic, and card hover exit clears both item/relic highlights. `play_scene current` launched `res://scenes/combat.tscn` successfully. Final public-readiness visual QA for real pointer hover across all six orb/card types and overlap/readability is transferred to ITCH-01/ITCH-08.
- Docs/wiki impact: `docs/test_plan.md`, `wiki/features.md`, and `wiki/log.md` were updated with CFR-09 code-complete behavior, validation evidence, and pending manual QA.
