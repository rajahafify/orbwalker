...
- Progress: Completed on `codex/cfr-08-enemy-intent-hp-preview`: `CombatPlayerController` computes projected incoming damage from the current enemy intent and visible player HP/armor, then passes that dictionary to `PlayerLoadoutHud`. The shared HUD creates runtime overlay nodes on the player HP bar: unblocked and partially blocked attacks show a red-to-empty-to-red blinking segment equal to projected HP loss, aligned to the right edge of the current HP fill; the transparent phase reveals an empty HP backing instead of the normal HP fill. Fully blocked attacks hide the HP-loss segment and pulse the armor badge/bar area instead. The combat controller also computes a presentation-only enemy intent preview dictionary with an `entries` array; today's supported entry is `kind="block"`, rendered as a blinking silver overshield strip above the enemy HP bar with width `intent.block / enemy.max_hp`. Hovering either the HP danger segment or enemy block preview emits UI-only enter/exit behavior that makes the existing enemy intent display scale larger and blink for as long as the pointer remains over the segment while writing a readable incoming-damage or incoming-block summary to the combat status/turn summary. This does not change combat math, enemy intent values, match resolver behavior, RunState routing, enemy attack resolution, replay timing, board resolve order, or `combat_speed`.
- Blockers: None.
...
- Acceptance:
  ...
  - Enemy block intent shows a silver overshield preview above the enemy HP bar scaled against enemy max HP.
  - Hovering the segment makes enemy intent easier to read until hover exits without acting in combat.
  - The preview does not permanently obscure player HP, armor, equipment, consumables, or mastery cards.
- Validation: `git diff --check` passed. Godot MCP reached Godot 4.6.2, loaded `player_loadout_hud.gd` and `combat_player_controller.gd`, instantiated `res://scenes/combat/combat_player.tscn`, confirmed expected HP/intent/HUD nodes exist, probed projected-damage math for unblocked (`attack=14`, `armor=0`, `hp_loss=14`), partially blocked (`attack=14`, `armor=9`, `hp_loss=5`), fully blocked (`attack=14`, `armor=14`, `hp_loss=0`, `fully_blocked=true`), overkill clamping (`attack=80`, `armor=5`, `hp_loss=40` at `40 HP`), and no-attack intent (`{}`). A focused `PlayerLoadoutHud` probe confirmed the danger segment appears for unblocked/partial cases, hides for full-block/no-attack cases, disables mouse interception when hidden, and shows the armor risk pulse only for full block. Follow-up validation confirmed the danger fill uses full red over an empty HP backing, the `mouse_entered` and `mouse_exited` hover signals are connected, and the old press callback was removed. A focused enemy block preview probe confirmed block intent produces a silver overshield strip at `block / max_hp` width and hides for no-block intent. `play_scene current` launched successfully. `get_godot_errors` still reports the two known stale CFR-02 enum reload diagnostics and no new CFR-08 runtime errors from the fresh scene smoke.
