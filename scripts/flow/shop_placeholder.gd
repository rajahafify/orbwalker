...
@onready var _shop_actions_row: BoxContainer = %ShopActionsRow
@onready var _bottom_actions_row: BoxContainer = %BottomActionsRow
...
func _apply_responsive_layout() -> void:
	...
	if viewport_size.y <= 0.0:
		return
	var is_compact := viewport_size.x / viewport_size.y < 1.2 or viewport_size.x < 980.0
	_shop_actions_row.vertical = is_compact
	_bottom_actions_row.vertical = is_compact
	_shop_actions_row.add_theme_constant_override("separation", 6 if is_compact else 8)
	_bottom_actions_row.add_theme_constant_override("separation", 6 if is_compact else 8)
