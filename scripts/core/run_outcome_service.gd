...
func boss_relic_reward_options_snapshot() -> Array[Dictionary]:
	return _owner._boss_relic_reward_options.duplicate(true)
...
func claim_boss_relic_reward(option_index: int) -> Dictionary:
	...
	if option_index < 0 or option_index >= _owner._boss_relic_reward_options.size():
		return {"ok": false, "reason": "invalid_option_index"}

	var option: Dictionary = _owner._boss_relic_reward_options[option_index]
	...
	_owner._boss_reward_claimed_relic_id = relic_id
	_owner._boss_relic_reward_options.clear()
	(
		_owner
		. _run_log_append(
			"boss_reward_choice",
...
func skip_boss_relic_reward() -> Dictionary:
	...
	if not _owner.is_current_step_boss_reward():
		return {"ok": false, "reason": "not_boss_reward_step"}
	_owner._boss_reward_claimed_relic_id = ""
	_owner._boss_relic_reward_options.clear()
	_owner._run_log_append("boss_reward_skip", {})
...
func advance_after_boss_reward() -> Dictionary:
	...
	_owner._advance_sequence("advance_after_boss_reward")
	return _owner._transition_result()
...
func mark_fight_victory() -> Dictionary:
	...
	if bool(_owner._current_encounter.get("is_boss", false)):
		_owner.bosses_defeated += 1
		if _owner.dungeon_level >= _owner.MAX_DUNGEON_LEVELS:
			finalize_run(true, "Final boss defeated.")
			return _owner._transition_result({"base_gold_reward": base_gold_reward})
		prepare_boss_relic_reward_options()
	_owner._advance_sequence("mark_fight_victory")
	return _owner._transition_result({"base_gold_reward": base_gold_reward})


func mark_player_defeated(cause: String) -> Dictionary:
	if _owner.run_active and _owner.is_current_step_fight():
		_owner._ensure_run_log_core_event_recorder().record_fight_end("defeat", _owner._current_encounter, cause)
...
func advance_after_shop(mark_skipped: bool) -> Dictionary:
	...
	(
		...
		. _run_log_append(
			"shop_leave",
			{
				...
				"shop_before": _owner._ensure_run_log_shop_event_recorder().sanitize_shop_snapshot(shop_before_close, _owner.run_gold),
				"shop_after": _owner._ensure_run_log_shop_event_recorder().sanitize_shop_snapshot(_owner.ensure_shop_state().to_snapshot(), _owner.run_gold),
			}
		)
	)
	if not _owner.run_active:
	...
	_owner._advance_sequence("advance_after_shop")
	return _owner._transition_result()


func run_summary_snapshot() -> Dictionary:
	if _owner._run_summary.is_empty():
	...
	return _owner._run_summary.duplicate(true)
...
func finalize_run(victory: bool, cause: String) -> void:
	...
	if victory:
		...
		victory_unlocks = _owner._ensure_profile_unlock_service().grant_victory_equipment_unlocks()
	elif _owner.is_current_step_fight():
		levels_cleared = _owner.dungeon_level - 1

	var score_added := 0
	if not _owner._run_score_banked:
		score_added = _owner.add_total_score(_owner.run_score)
		_owner._run_score_banked = true
	var meta_snapshot: Dictionary = _owner.meta_profile_snapshot()
	...
	_owner._ensure_run_log_core_event_recorder().record_run_end(victory, cause, _owner._run_summary)
	if _owner._ensure_run_logger().should_export_run_log_files():
		_owner._run_log_export_to_disk()
	_owner._emit_run_state_signals(signal_before, "finalize_run", "")
