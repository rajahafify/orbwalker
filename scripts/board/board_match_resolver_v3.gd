...
func get_match_groups(board_state: BoardState) -> Array[Dictionary]:
	return _find_match_groups(board_state)


func resolve_all(board_state: BoardState, max_steps: int = 64) -> Dictionary:
	var result := {
		"total_combos": 0,
		"matched_counts": {},
		"passes": [],
	...
	for step_index in max_steps:
		var groups: Array[Dictionary] = _find_match_groups(board_state)
		if groups.is_empty():
			break
		...
		var cleared_cells: Array[Vector2i] = _clear_groups(board_state, groups)
		cells_cleared.emit(cleared_cells)

		var fall_moves: Array[Dictionary] = _apply_gravity(board_state)
		gravity_applied.emit(fall_moves)

		var refill_spawns: Array[Dictionary] = _refill_empty_cells(board_state)
...
func _find_match_groups(board_state: BoardState) -> Array[Dictionary]:
	var line_cells_by_orb: Dictionary = _collect_line_match_cells(board_state)
...
func _collect_line_match_cells(board_state: BoardState) -> Dictionary:
	var line_cells_by_orb := {}
	for orb_id in OrbType.ALL_TYPES:
		line_cells_by_orb[orb_id] = {}

	for row in BoardState.ROW_COUNT:
		var run_orb := board_state.get_cell(0, row)
		var run_start := 0
		var run_length := 1
		for column in range(1, BoardState.COLUMN_COUNT):
			var next_orb := board_state.get_cell(column, row)
	...
	for column in BoardState.COLUMN_COUNT:
		var run_orb := board_state.get_cell(column, 0)
		var run_start := 0
		var run_length := 1
		for row in range(1, BoardState.ROW_COUNT):
			var next_orb := board_state.get_cell(column, row)
			if next_orb == run_orb and OrbType.is_valid_id(run_orb):
				run_length += 1
				continue
...
func _collect_component_indices(cells: Dictionary, start_index: int, visited: Dictionary) -> PackedInt32Array:
	...
	while cursor < stack.size():
		...
		var x: int = current % BoardState.COLUMN_COUNT
		var y: int = floori(float(current) / float(BoardState.COLUMN_COUNT))
		_try_push_neighbor_index(cells, _index(x - 1, y), x > 0, visited, stack)
		_try_push_neighbor_index(cells, _index(x + 1, y), x < BoardState.COLUMN_COUNT - 1, visited, stack)
		_try_push_neighbor_index(cells, _index(x, y - 1), y > 0, visited, stack)
		_try_push_neighbor_index(cells, _index(x, y + 1), y < BoardState.ROW_COUNT - 1, visited, stack)
...
func _clear_groups(board_state: BoardState, groups: Array[Dictionary]) -> Array[Vector2i]:
	var unique_cells := {}
	for group in groups:
	...
	for index_variant in unique_cells.keys():
		...
		board_state.clear_cell(cell.x, cell.y)
		cleared_cells.append(cell)
	return cleared_cells


func _apply_gravity(board_state: BoardState) -> Array[Dictionary]:
	var fall_moves: Array[Dictionary] = []

	for column in BoardState.COLUMN_COUNT:
		var write_row := BoardState.ROW_COUNT - 1
		for row in range(BoardState.ROW_COUNT - 1, -1, -1):
			var orb_id := board_state.get_cell(column, row)
			if orb_id == BoardState.EMPTY_ORB_ID:
				continue

			if write_row != row:
				board_state.set_cell(column, write_row, orb_id)
				board_state.clear_cell(column, row)
				fall_moves.append({
					"orb_id": orb_id,
		...
		for clear_row in range(write_row, -1, -1):
			board_state.clear_cell(column, clear_row)

	return fall_moves


func _refill_empty_cells(board_state: BoardState) -> Array[Dictionary]:
	var refill_spawns: Array[Dictionary] = []

	for column in BoardState.COLUMN_COUNT:
		for row in BoardState.ROW_COUNT:
			if board_state.get_cell(column, row) != BoardState.EMPTY_ORB_ID:
				continue

			var spawned_orb := board_state.roll_random_orb()
			board_state.set_cell(column, row, spawned_orb)
...
func _coords(index: int) -> Vector2i:
	return Vector2i(
		index % BoardState.COLUMN_COUNT,
		floori(float(index) / float(BoardState.COLUMN_COUNT))
	)
