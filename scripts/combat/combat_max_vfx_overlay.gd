...
func _spawn_status_replay_recipe(kind: String, center: Vector2, max_size: float, base_size: float, duration: float, intensity: int, screen_wide: bool) -> void:
	...
	match clean_kind:
		...
		"earth":
			_spawn_status_flipbook("poison", center + Vector2(0.0, max_size * 0.12), status_size * Vector2(0.48, 0.36), duration * 0.98, Color(0.72, 1.0, 0.30, 0.62), 0.0, Vector2(0.0, -max_size * 0.05), 1.10, 1.8, 0.0, 1)
			_spawn_status_flipbook("weaken", center + Vector2(0.0, max_size * 0.04), status_size * Vector2(0.36, 0.30), duration * 0.64, Color(0.44, 0.92, 0.22, 0.44), 0.10, Vector2.ZERO, 0.94, 2.0, 0.18, 1)
			_spawn_tornado_scene(center + Vector2(-max_size * 0.08, max_size * 0.18), status_size * Vector2(0.84, 0.58), duration * 0.86, intensity, 0.08, Vector2(max_size * 0.14, -max_size * 0.06), 1.7, "tornado poison")
...
func _spawn_status_cast_recipe(kind: String, source: Vector2, target: Vector2, delta: Vector2, spool_size: Vector2, spool_lifetime: float, travel_lifetime: float, intensity: int, core: Color, accent: Color) -> void:
	...
	match clean_kind:
		...
		"ice":
			...
			_spawn_beam_effect(source, delta, "ice", travel_duration * 0.92, intensity, launch_delay, 0.78)
			for lane_index in [-1, 1]:
		...
		"earth":
			_spawn_status_flipbook("poison", source + Vector2(0.0, 16.0), spool_size * Vector2(0.58, 0.42), spool_duration * 0.94, Color(0.72, 1.0, 0.30, 0.58), 0.0, Vector2.ZERO, 1.10, 1.0, angle, 1)
			_spawn_tornado_scene(source + Vector2(0.0, 22.0), spool_size * Vector2(0.78, 0.48), spool_duration, intensity, 0.08, Vector2.ZERO, 1.4, "tornado poison")
			_spawn_status_path_afterimage("earth", source, delta, launch_delay * 0.74, travel_duration * 1.30, intensity + 1, angle)
			_spawn_status_flipbook("poison", source, travel_size * Vector2(0.48, 0.32), travel_duration * 1.04, Color(0.60, 0.96, 0.28, 0.42), launch_delay, delta, 0.90, 2.2, angle, 1)
			_spawn_tornado_scene(target + Vector2(0.0, 18.0), spool_size * Vector2(0.92, 0.56), travel_duration * 1.02, intensity, launch_delay + travel_duration * 0.86, Vector2.ZERO, 2.6, "tornado poison")
			_spawn_status_flipbook("weaken", target + Vector2(0.0, 10.0), spool_size * (0.42 + float(intensity) * 0.018), travel_duration * 0.92, Color(0.52, 1.0, 0.24, 0.46), launch_delay + travel_duration * 0.90, Vector2.ZERO, 1.08, 3.0, angle, 1)
		"heart":
...
func _spawn_tornado_scene(center_local: Vector2, draw_size: Vector2, lifetime: float, intensity: int, delay: float = 0.0, move_offset: Vector2 = Vector2.ZERO, z: float = 0.0, keep_child_name: String = "") -> Node3D:
	...
	_animate_imported_node(effect, lifetime, delay, move_offset, 1.12, 0.28)
	return effect
...
func _elemental_kind_colors(kind: String) -> Dictionary:
	match _clean_kind(kind):
		...
		"earth":
			return {"primary": Color(0.82, 1.0, 0.36, 1.0), "secondary": Color(0.28, 0.86, 0.20, 1.0), "tertiary": Color(0.08, 0.24, 0.06, 1.0)}
		"heart":
...
func _kind_colors(kind: String) -> Dictionary:
	match _clean_kind(kind):
		...
		"earth":
			return {"accent": Color(0.45, 0.92, 0.22, 1.0), "core": Color(0.90, 1.0, 0.38, 1.0)}
		"heart":
