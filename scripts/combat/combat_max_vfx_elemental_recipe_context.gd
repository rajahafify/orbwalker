extends RefCounted
class_name CombatMaxVfxElementalRecipeContext
...
func spawn_replay_recipe(kind: String, center: Vector2, max_size: float, base_size: float, duration: float, intensity: int, screen_wide: bool) -> void:
	var clean_kind := clean_kind(kind)
	var area_size := Vector2(base_size, base_size) * (1.35 if screen_wide else 0.86)
	match clean_kind:
		"fire":
			spawn_elemental_effect("area", center, "fire", area_size * 1.10, duration * 1.32, intensity, 0.0, Vector2.ZERO, 0.0, 0.45, 0.96)
			spawn_elemental_effect(
				"projectile",
				center + Vector2(-max_size * 0.62, max_size * 0.16),
				"fire",
				Vector2(base_size * 0.88, base_size * 0.36),
				duration * 0.54,
				intensity,
				0.02,
				Vector2(max_size * 1.18, -max_size * 0.18),
				-0.12,
				1.8,
				0.76
			)
			spawn_elemental_effect(
				"cast", center + Vector2(0.0, -max_size * 0.22), "fire", area_size * 0.56, duration * 0.82, intensity, 0.06, Vector2.ZERO, 0.0, 1.4, 0.72
			)
			spawn_pack_layer(
				pack_impact_scene_key("fire", intensity, screen_wide), center, "fire", area_size * 0.52, duration * 0.58, intensity, 0.05, 0.10, 2.0, 0.62
			)
			spawn_pack_layer(
				"hit_01", center + Vector2(max_size * 0.12, -max_size * 0.08), "fire", area_size * 0.32, duration * 0.44, intensity, 0.14, -0.35, 2.4, 0.58
			)
		"ice":
			spawn_elemental_effect("area", center, "ice", area_size * 0.92, duration * 1.48, intensity, 0.0, Vector2.ZERO, 0.0, 0.40, 0.86)
			spawn_elemental_effect(
				"projectile",
				center + Vector2(-max_size * 0.58, -max_size * 0.12),
				"ice",
				Vector2(base_size * 0.74, base_size * 0.28),
				duration * 0.70,
				intensity,
				0.04,
				Vector2(max_size * 1.08, max_size * 0.08),
				-0.04,
				1.7,
				0.60
			)
			spawn_elemental_effect(
				"projectile",
				center + Vector2(max_size * 0.46, max_size * 0.10),
				"ice",
				Vector2(base_size * 0.48, base_size * 0.22),
				duration * 0.62,
				intensity,
				0.13,
				Vector2(-max_size * 0.52, -max_size * 0.04),
				PI - 0.10,
				1.6,
				0.42
			)
			spawn_elemental_effect(
				...
				intensity,
				0.10,
				Vector2(0.0, max_size * 0.14),
				0.0,
				1.2,
				0.52
			)
			spawn_pack_layer("impact_02", center, "ice", area_size * 0.48, duration * 0.74, intensity, 0.10, 0.0, 2.1, 0.50)
			spawn_pack_layer(
				"hit_02", center + Vector2(-max_size * 0.10, -max_size * 0.10), "ice", area_size * 0.28, duration * 0.50, intensity, 0.20, 0.45, 2.5, 0.42
			)
		"earth":
			var ground_effect := spawn_elemental_effect(
				"area", center + Vector2(0.0, max_size * 0.16), "earth", area_size * 0.92, duration * 1.50, intensity, 0.0, Vector2.ZERO, 0.0, 0.28, 0.82
			)
			stretch_effect(ground_effect, Vector3(1.55, 0.52, 1.0))
			for i in range(5 + mini(5, intensity)):
				...
				var crack := spawn_elemental_effect(
					"area",
					center + offset,
					"earth",
					area_size * Vector2(0.20, 0.16),
					duration * 0.52,
					intensity,
					duration * (0.04 + progress * 0.18),
					Vector2.ZERO,
					sin(float(i)) * 0.42,
					1.1,
					0.54
				)
				stretch_effect(crack, Vector3(1.80, 0.38, 1.0))
			spawn_pack_layer("impact_01", center + Vector2(0.0, max_size * 0.10), "earth", area_size * 0.44, duration * 0.72, intensity, 0.16, 0.0, 2.0, 0.44)
		"heart":
			spawn_elemental_effect(
				"area", center, "heart", area_size * Vector2(0.78, 1.10), duration * 1.44, intensity, 0.0, Vector2(0.0, -max_size * 0.16), 0.0, 0.45, 0.76
			)
			for i in range(4 + mini(4, intensity)):
				var x := (float(i) - 1.5) * max_size * 0.18
				spawn_elemental_effect(
					"cast",
					center + Vector2(x, max_size * 0.28),
					"heart",
					area_size * 0.22,
					duration * 0.86,
					intensity,
					duration * (0.03 + float(i) * 0.035),
					Vector2(0.0, -max_size * (0.42 + float(i % 2) * 0.12)),
					0.0,
					1.5,
					0.52
				)
			spawn_pack_layer("hit_02", center + Vector2(0.0, -max_size * 0.06), "heart", area_size * 0.30, duration * 0.58, intensity, 0.18, 0.0, 2.3, 0.36)
		"armor":
			var shell := spawn_elemental_effect(
				"area", center, "armor", area_size * Vector2(1.06, 0.86), duration * 1.52, intensity, 0.0, Vector2.ZERO, 0.0, 0.55, 0.78
			)
			stretch_effect(shell, Vector3(1.25, 0.74, 1.0))
			spawn_elemental_effect(
				"cast", center + Vector2(0.0, -max_size * 0.12), "armor", area_size * 0.38, duration * 0.82, intensity, 0.08, Vector2.ZERO, 0.0, 1.5, 0.54
			)
			spawn_pack_layer("impact_02", center, "armor", area_size * 0.48, duration * 0.64, intensity, 0.10, 0.0, 2.1, 0.46)
		"gold":
			spawn_elemental_effect("area", center, "gold", area_size * 0.92, duration * 1.24, intensity, 0.0, Vector2.ZERO, 0.0, 0.42, 0.78)
			for i in range(4 + mini(5, intensity)):
				...
				spawn_elemental_effect(
					...
					intensity,
					duration * (0.03 + float(i) * 0.035),
					Vector2(sin(float(i)) * max_size * 0.18, max_size * 0.30),
					0.0,
					1.6,
					0.58
				)
			spawn_coin_rain(center, max_size, duration, intensity, screen_wide)
			spawn_pack_layer("hit_01", center, "gold", area_size * 0.34, duration * 0.50, intensity, 0.12, 0.22, 2.2, 0.48)
		_:
			spawn_elemental_effect("area", center, kind, area_size, duration * 1.30, intensity, 0.0, Vector2.ZERO, 0.0, 0.45, 0.92)
	if screen_wide:
...
func spawn_path_afterimage(kind: String, source: Vector2, delta: Vector2, launch_delay: float, travel_duration: float, intensity: int, angle: float) -> void:
	...
	for i in range(count):
		...
		if clean_kind == "earth":
			...
			alpha = 0.38
		spawn_elemental_effect("area", point, clean_kind, size, travel_duration * 0.44, intensity, delay, Vector2.ZERO, angle, 0.2, alpha)
...
func spawn_screen_wide(kind: String, center: Vector2, lifetime: float, intensity: int) -> void:
	...
	spawn_elemental_effect("area", focus, clean_kind, size, lifetime * 1.36, intensity, 0.0, Vector2.ZERO, 0.0, -0.8, 0.64)
	...
	for i in range(bursts):
		...
		spawn_elemental_effect("cast", Vector2(x, y), clean_kind, size * 0.22, lifetime * 0.74, intensity, delay, Vector2.ZERO, sin(float(i)) * 0.28, 1.4, 0.42)
...
func spawn_beam_layers(kind: String, source: Vector2, delta: Vector2, lifetime: float, intensity: int, angle: float) -> void:
	...
	match clean_kind:
		"ice":
			...
			spawn_elemental_effect(
				"projectile", source, "ice", Vector2(178 + intensity * 14, 68 + intensity * 6), lifetime * 1.26, intensity, 0.0, delta, angle - PI, 1.5, 0.68
			)
			spawn_elemental_effect(
				"projectile",
				source + normal * 11.0,
				"ice",
				Vector2(160 + intensity * 12, 64 + intensity * 6),
				lifetime * 1.22,
				intensity,
				0.03,
				delta - normal * 8.0,
			...
			spawn_elemental_effect(
				...
				source - normal * 11.0,
				"ice",
				Vector2(134 + intensity * 10, 54 + intensity * 5),
				lifetime * 1.16,
				intensity,
				0.07,
				delta + normal * 8.0,
		...
		"earth":
			...
			var crawl := spawn_elemental_effect(
				"projectile", source, "earth", Vector2(146 + intensity * 13, 62 + intensity * 6), lifetime * 1.30, intensity, 0.0, delta, angle - PI, 1.25, 0.56
			)
			stretch_effect(crawl, Vector3(1.35, 0.68, 1.0))
		"heart":
			spawn_elemental_effect(
				"projectile", source, "heart", Vector2(136 + intensity * 10, 82 + intensity * 7), lifetime * 1.22, intensity, 0.0, delta, angle - PI, 1.4, 0.60
			)
		"armor":
			spawn_elemental_effect(
				"projectile", source, "armor", Vector2(142 + intensity * 10, 88 + intensity * 7), lifetime * 1.20, intensity, 0.0, delta, angle - PI, 1.4, 0.62
			)
		"gold":
			spawn_elemental_effect(
				"projectile", source, "gold", Vector2(142 + intensity * 10, 76 + intensity * 6), lifetime * 1.14, intensity, 0.0, delta, angle - PI, 1.4, 0.64
			)
		_:
			spawn_elemental_effect(
				"projectile",
				source,
				clean_kind,
				Vector2(184 + intensity * 16, 98 + intensity * 8),
				lifetime * 1.22,
				intensity,
				0.0,
				delta,
