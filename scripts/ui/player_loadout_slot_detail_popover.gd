...
const CALLBACK_SELL_SLOT_REQUESTED := "sell_slot_requested"

var _hud_nodes_provider: Callable
var _player_data_provider: Callable
var _selected_equipment_slot_provider: Callable
var _selected_consumable_slot_provider: Callable
var _content_lookup: Callable
var _apply_rect: Callable
...
func bind(dependencies: Dictionary, callbacks: Dictionary = {}) -> void:
	...
	_selected_equipment_slot_provider = dependencies.get("selected_equipment_slot_provider", Callable())
	_selected_consumable_slot_provider = dependencies.get("selected_consumable_slot_provider", Callable())
	_content_lookup = dependencies.get("content_lookup", Callable())
	_apply_rect = dependencies.get("apply_rect", Callable())
...
func _ensure_slot_detail_popover() -> void:
	if _slot_detail_bubble != null:
		return
	var nodes := _hud_nodes()
	var parent := nodes.get("popover_parent", null) as Control
	if parent == null:
		parent = nodes.get("section", null) as Control
	if parent == null:
		return
	_slot_detail_bubble = Panel.new()
	_slot_detail_bubble.name = "SlotDetailBubble"
	_slot_detail_bubble.visible = false
	_slot_detail_bubble.z_index = int(nodes.get("popover_z_index", 210))
...
func _update_selected_slot_popover() -> void:
	...
	if selected_kind == "":
		...
		return
	var slot_index := _selected_slot_index(selected_kind)
	var nodes := _hud_nodes()
	var row := nodes.get("equipment_icons") as Control
	if selected_kind == "consumable":
		row = nodes.get("consumable_icons") as Control
...
func _update_slot_detail_bubble() -> void:
	...
	_apply_rect_call(_slot_detail_title, Rect2(Vector2(SLOT_DETAIL_BUBBLE_INTERNAL_PADDING, cursor_y), Vector2(content_width, title_height)))
	cursor_y += title_height
	if has_description:
		cursor_y += row_gap
		_slot_detail_description.visible = true
		_apply_rect_call(_slot_detail_description, Rect2(Vector2(SLOT_DETAIL_BUBBLE_INTERNAL_PADDING, cursor_y), Vector2(content_width, description_height)))
		cursor_y += description_height
	else:
		_slot_detail_description.visible = false
		_apply_rect_call(_slot_detail_description, Rect2(Vector2(SLOT_DETAIL_BUBBLE_INTERNAL_PADDING, cursor_y), Vector2(content_width, 0.0)))
	...
	if show_sell_action:
		cursor_y += 12.0
		_apply_rect_call(_slot_detail_sell_button, Rect2(Vector2(SLOT_DETAIL_BUBBLE_INTERNAL_PADDING, cursor_y), Vector2(content_width, sell_button_height)))
	else:
		_apply_rect_call(_slot_detail_sell_button, Rect2(Vector2(SLOT_DETAIL_BUBBLE_INTERNAL_PADDING, cursor_y), Vector2(content_width, 0.0)))
...
func _slot_detail_popover_width(parent_width: float) -> float:
	...
	var title_len := float(_slot_detail_title.text.length()) if _slot_detail_title != null else 0.0
	var description_len := float(_slot_detail_description.text.length()) if _slot_detail_description != null else 0.0
...
func _slot_popover_shows_sell_action() -> bool:
	...
	if selected_kind == "":
		return false
	return selected_kind == _hover_slot_type and _selected_slot_index(selected_kind) == _hover_slot_index
...
func _selected_slot_sell_text() -> String:
	...
	if selected_kind == "":
		return "Select slot"
	var item_id := _slot_content_id(selected_kind, _selected_slot_index(selected_kind))
	var content := _lookup_content_definition(item_id)
...
func _on_slot_detail_sell_pressed() -> void:
	...
	if selected_kind == "":
		return
	_call(CALLBACK_SELL_SLOT_REQUESTED, [selected_kind, _selected_slot_index(selected_kind)])
...
func _is_slot_detail_sell_button(global_point: Vector2) -> bool:
	return _control_contains_point(_slot_detail_sell_button, global_point)
...
func _selected_equipment_slot() -> int:
	return int(_selected_equipment_slot_provider.call()) if _selected_equipment_slot_provider.is_valid() else -1


func _selected_consumable_slot() -> int:
	return int(_selected_consumable_slot_provider.call()) if _selected_consumable_slot_provider.is_valid() else -1


func _selected_slot_index(slot_type: String) -> int:
	return _selected_equipment_slot() if slot_type == "equipment" else _selected_consumable_slot()


func _lookup_content_definition(content_id: String) -> Dictionary:
	if _content_lookup.is_valid():
		var value: Variant = _content_lookup.call(content_id)
		if value is Dictionary:
			return value
	return {"display_name": content_id, "description": "", "icon_key": ""}


func _apply_rect_call(control: Control, rect: Rect2) -> void:
	if _apply_rect.is_valid():
		_apply_rect.call(control, rect)
		return
	if control != null:
