...
- `scripts/combat/` - Combat state machine, player state, enemy state, the combat scene controller, and the outcome overlay presentation helper. `combat_player_controller.gd` keeps combat orchestration, RunState routing, debug commands, and resolver replay timing; `combat_outcome_overlay.gd` owns standard outcome card and boss reward overlay presentation state/layout. (source: `scripts/combat/combat_state_machine.gd`, `scripts/combat/combat_player_controller.gd`, `scripts/combat/combat_outcome_overlay.gd`)
