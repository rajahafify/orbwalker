...
- `scripts/core/` - Global run orchestration and shared startup logic. `run_state.gd` is the main autoload-backed controller, `main_boot.gd` handles the main menu launch, and `audio_manager.gd` owns music/SFX playback with exported WAV music preferred over code-generated fallback loops. Android/template builds prefer imported `res://` audio streams before direct source-WAV decoding so packaged builds do not depend on absolute source paths. (source: `scripts/core/run_state.gd`, `scripts/core/main_boot.gd`, `scripts/core/audio_manager.gd`, `resources/audio/music/`)
...
- `resources/audio/music/` - Godot-ready WAV music rendered from MIDI sources for menu, combat, shop, credit, and melody candidates; runtime music loading opens these source WAVs directly before falling back to imported resources. (source: `resources/audio/music/`, `tools/audio/export_midi_to_wav.py`, `scripts/core/audio_manager.gd`)
- `raw/` - Raw source assets that are not edited by agents unless explicitly requested, currently including MIDI music sources and `GeneralUser GS v1.471.sf2` for music export. (source: `raw/`, `tools/audio/export_midi_to_wav.py`)
