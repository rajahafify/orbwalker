...
- Combat now has a dedicated reference-style `ElementalMasteryPanel` between the board and player HUD. It renders six large mastery cards inside a generated ornate panel frame with a centered title, main-menu mastery iconography, title, level, and transient `+N DAMAGE`, `+N HEAL`, `+N ARMOR`, or `+N GOLD` feedback in the lower card slot; after cascades finish, card effects replay left-to-right from `turn_log` with beams/shell/impact VFX, block-before-HP enemy damage, player heal/armor/gold feedback, and enemy attack armor-before-HP replay. (source: `scenes/combat/combat_player.tscn`, `scripts/combat/combat_player_controller.gd`, `scripts/ui/player_loadout_hud.gd`, `scripts/ui/visual_registry.gd`, `resources/art/first_pass/derived/icons/mastery_fire.png`, `resources/art/first_pass/derived/ui_chrome/`, `resources/art/first_pass/derived/vfx/`)
