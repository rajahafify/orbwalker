...
- Combat board now shows floating combo text near each resolved match group during cascades, with combo count increasing by resolve order for a more readable and energetic post-drag feedback loop. (source: `scripts/combat/combat_player_controller.gd`, `scripts/board/board_match_resolver_v3.gd`)
