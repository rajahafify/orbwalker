...
- Player loadout now renders as two framed bottom groups (`EQUIPMENT` and `CONSUMABLES`) with centered headers and 64px slots, while relic/mastery rows stay hidden unless runtime state enables them. (source: `scripts/combat/combat_player_controller.gd`, `scenes/combat/combat_player.tscn`)
