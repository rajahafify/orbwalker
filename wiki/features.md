...
- Combat board now shows one floating combo counter that updates from `COMBO x1` upward through each visible resolve/cascade step instead of stacking multiple match-group popups. Drag release keeps a cloned pre-resolve board on screen and advances that visual board through clear, gravity, and refill steps so the first match flash is visible before new orbs appear. Presentation is trigger-based: a visible match triggers the combo counter update, and that combo update triggers the matching Elemental Mastery preview. The post-drag presentation has a hidden speed setting (`slow`, `fast`, `instant`) that defaults to `slow` so cascade count changes, clears, gravity/refill, and turn replay VFX are easier to read. (source: `scripts/combat/combat_player_controller.gd`, `scripts/board/board_match_resolver_v3.gd`)
