...
- Combat now has a dedicated reference-style `ElementalMasteryPanel` between the board and player HUD. The selected live layout is the standalone preview's variant 5, `Feedback Ready`: a `1048 x 368` ornate panel with full-width title band, six centered `156 x 222` cards, preview panel/card chrome, real main-menu mastery iconography, readable name/level lanes, and a reserved lower feedback lane for transient `+N DAMAGE`, `+N HEAL`, `+N ARMOR`, or `+N GOLD` text. After cascades finish, card effects replay left-to-right from `turn_log` with beams/shell/impact VFX, block-before-HP enemy damage, player heal/armor/gold feedback, and enemy attack armor-before-HP replay. (source: `scenes/combat/combat_player.tscn`, `scripts/combat/combat_player_controller.gd`, `scripts/ui/player_loadout_hud.gd`, `scripts/ui/visual_registry.gd`, `resources/art/first_pass/derived/icons/mastery_fire.png`, `resources/art/first_pass/derived/ui_chrome/mastery_preview_panel_frame.png`, `resources/art/first_pass/derived/vfx/`, `docs/tmp_elemental_mastery_visual_issues.md`)
