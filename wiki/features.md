...
- Player panel now follows a simplified reference bottom-HUD structure with a larger hero portrait card, level badge, long primary HP bar, large always-visible equipment and consumable rails, and a full-width mastery strip with icon-plus-number cells; armor, stat chips, combat meta, and turn summary rows are hidden from the player panel. (source: `scenes/combat/combat_player.tscn`, `scripts/combat/combat_player_controller.gd`)
