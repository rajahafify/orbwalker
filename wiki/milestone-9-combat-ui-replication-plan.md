...
- Scope is combat scene UI only for this pass. Shop and reward scenes are out of scope for this specific plan. (source: `scripts/combat/combat_player_controller.gd`)
- The blue secondary bar in the reference is treated as styling, not a new mana mechanic; no new combat resource is introduced. (source: `scripts/combat/combat_player_controller.gd`)
...
- Existing runtime data remains the source for all shown values (run progress, hp/armor/gold, timer, enemy intent/hp, combo summary, progression slots). (source: `scripts/combat/combat_player_controller.gd`)
...
- `scenes/combat/combat_player.tscn` - Primary combat HUD structure to reshape to reference composition.
- `scripts/combat/combat_player_controller.gd` - HUD data binding, formatting, responsive behavior, and visual style wiring.
