...
## 2026-05-06 - Board MVC boundary follow-up

- Tightened the board MVC API boundary after the naming refactor: BoardView now exposes presentation helpers for model display, drag visuals, hover lookup, board presentation clearing, and input filtering, while BoardController owns board initialization, debug state access, board mutation helpers, consumable orb conversion, visual/simulation resolve model preparation, and final model commit. Combat still owns combat phase authority, resolver orchestration, combat math, RunState routing, VFX/HUD, and status/log text, but no longer writes board-view internals or mutates board cells directly for consumables. Godot MCP script reloads and main-scene smoke reported no session errors; editor-script scene-instance probes remain limited by non-tool placeholder instances. (source: scripts/board/board_view.gd, scripts/board/board_controller.gd, scripts/combat/combat_player_controller.gd, docs/test_plan.md)

## 2026-05-06 - Visual resolve commit boundary extraction

- Moved visual resolve board-model mutations for clear, gravity, and refill from `combat_resolve_presenter.gd` into new `BoardController` APIs (`apply_visual_clear_groups`, `apply_visual_fall_moves`, `apply_visual_refill_spawns`) and injected `BoardController` into presenter bind so replay timing/presentation stays in presenter while board-domain mutation ownership stays in board controller. Updated [[architecture]] and [[file-map]] ownership text to reflect board-controller delegated visual commits. Godot MCP `get_project_info`, `view_script` on changed runtime files, and `get_godot_errors` ran; no new parse/runtime errors were introduced by this slice, and existing session warnings were pre-existing enum/seed warnings. (source: `scripts/board/board_controller.gd`, `scripts/combat/combat_resolve_presenter.gd`, `scripts/combat/combat_player_controller.gd`, `wiki/architecture.md`, `wiki/file-map.md`)

## 2026-05-06 - Shop MVC refactor follow-up

- Updated shop runtime ownership to a local MVC split and documented it in [[architecture]], [[file-map]], [[features]], and `docs/test_plan.md`: `ShopModel` now owns local slot/status/transition state plus enriched offer projection for rendering, `ShopView` now owns shop UI/layout/rendering and shared `PlayerLoadoutHud` binding/signals, `ShopController` now owns shop actions, RunState scene transitions with rollback callbacks, FlowTrace marks, and shop audio hooks, and `scripts/scenes/shop.gd` is now a thin host that wires scene nodes and forwards lifecycle/input/layout events. (source: `scripts/shop/shop_model.gd`, `scripts/shop/shop_view.gd`, `scripts/shop/shop_controller.gd`, `scripts/scenes/shop.gd`, `docs/test_plan.md`)
- Updated `player_hud_contract_probe.gd` contract notes to include shop host/view preload and shared HUD instancing checks against `scripts/shop/shop_view.gd` while preserving existing combat/shop shared HUD key checks. (source: `scripts/debug/player_hud_contract_probe.gd`)
- Validation recorded in `docs/test_plan.md`: Godot MCP `get_project_info`, `view_script` for touched shop/probe scripts, `play_scene`/`stop_running_scene` smoke, rerun `get_godot_errors`, and `git diff --check` passed with no session errors. (source: `docs/test_plan.md`)

## 2026-05-06 - Shop MVC active-entry crash fix

- Fixed the active-shop crash after the MVC refactor by removing an incorrect `Dictionary` type from the `ShopView._render_build_panel(...)` local `player_state` value; the snapshot contains the real `PlayerState` object and the shared `PlayerLoadoutHud` expects that object in its update payload. Godot MCP `view_script`, `play_scene current`, rerun `get_godot_errors`, and `git diff --check` passed after the fix. User QA passed after the crash fix. (source: `scripts/shop/shop_view.gd`, `docs/test_plan.md`)

## 2026-05-06 - Non-combat MVC ownership hardening

- Reworked the in-progress non-combat MVC split so model/view/controller ownership is real instead of stubbed. `MainMenuModel` now owns menu asset/profile snapshot formatting helpers, `MainMenuView` owns node binding plus menu rendering/layout/chrome/status/profile overlay presentation, and `MainMenuController` now focuses on lifecycle, Start Run/Collection/profile/reset actions, audio commands, FlowTrace transitions, and transition locks. (source: `scripts/main_menu/main_menu_model.gd`, `scripts/main_menu/main_menu_view.gd`, `scripts/main_menu/main_menu_controller.gd`, `scripts/scenes/main_menu.gd`)
- Applied the same ownership boundary for Collection and final run summary: `CollectionModel` owns unlock/state snapshots and claim validation helpers, `CollectionView` owns card rendering/chrome/status/toast presentation, `CollectionController` owns claim/back action routing and transition lock; `RunSummaryModel` owns summary/unlock formatting, `RunSummaryView` owns full summary surface rendering/chrome/status/toast presentation, and `RunSummaryController` owns New Run/Main Menu routing and transition rollback handling. (source: `scripts/collection/collection_model.gd`, `scripts/collection/collection_view.gd`, `scripts/collection/collection_controller.gd`, `scripts/scenes/collection.gd`, `scripts/run_summary/run_summary_model.gd`, `scripts/run_summary/run_summary_view.gd`, `scripts/run_summary/run_summary_controller.gd`, `scripts/scenes/run_summary.gd`)
- Updated wiki ownership references in [[architecture]], [[file-map]], and [[features]] to reflect the non-combat MVC boundaries and thin scene-host scripts. (source: `wiki/architecture.md`, `wiki/file-map.md`, `wiki/features.md`)
- Validation: Godot MCP bridge (`http://localhost:3571`) `view_script` loaded all touched non-combat MVC scripts and scene hosts; focused `execute_editor_script` instantiated `res://scenes/main_menu.tscn`, `res://scenes/collection.tscn`, and `res://scenes/run_summary.tscn`; `get_godot_errors` reported no current open-script parse errors and `Session has no errors` in debugger status. User manual QA passed after the refactor.

## 2026-05-06 - AR-19 through AR-23 architecture debt batch

- Implemented the M12 architecture debt follow-up through the multi-agent workflow: shop/content safety clamps and validation hardening, shared `FlowResultUtils`, combat phase reset encapsulation, `RunLogger` extraction behind the `RunState` facade, and `SceneRouter` extraction behind the `RunState` facade. (source: `scripts/content/content_registry.gd`, `scripts/shop/shop_service.gd`, `scripts/core/flow_result_utils.gd`, `scripts/combat/combat_state_machine.gd`, `scripts/core/run_logger.gd`, `scripts/core/scene_router.gd`, `scripts/core/run_state.gd`)
- Updated [[architecture]], [[file-map]], and [[known-issues]] to reflect that `RunState` remains the compatibility boundary while `RunLogger` owns run-log storage/export state and `SceneRouter` owns FlowTrace/transition backing state. (source: `wiki/architecture.md`, `wiki/file-map.md`, `wiki/known-issues.md`)
- Validation: explorer review was accepted at `8.6/10`; the worker first pass was rejected at `7.4/10`; the corrected worker pass was accepted at `8.7/10`. Godot MCP probes covered shop/content safety, shared flow-result semantics, phase-source shape, run-log snapshot/restore, and SceneRouter route retention/snapshot behavior. Final main-scene smoke and `get_godot_errors` were run after docs/wiki sync. (source: `docs/architecture_review_tasks.md`, `docs/test_plan.md`)

## 2026-05-07 - AR-24 ProfileRepository extraction

- Extracted profile/meta-profile file persistence from `RunState` into `ProfileRepository`, which now owns profile path constants, `ConfigFile` load/save, legacy meta-profile migration, save warnings, and last-I/O probe metadata. `RunState` remains the public facade for profile/meta snapshots, reset, equipment unlocks, recent unlock consumption, Total Score, and run lifecycle ordering. (source: `scripts/core/profile_repository.gd`, `scripts/core/run_state.gd`)
- Updated [[architecture]] and [[file-map]] for the new helper ownership and recorded the validation in `docs/test_plan.md` and `docs/architecture_review_tasks.md`. (source: `wiki/architecture.md`, `wiki/file-map.md`, `docs/test_plan.md`, `docs/architecture_review_tasks.md`)
- Validation: worker patch accepted at `8.8/10`; `git diff --check`, Godot MCP `view_script`, focused profile persistence probe, `play_scene main`, `stop_running_scene`, and `get_godot_errors` passed with no session errors. (source: `docs/architecture_review_tasks.md`, `docs/test_plan.md`)

## 2026-05-07 - AR-25 BalanceManager extraction

- Extracted prototype balance defaults, backing lever state, normalization, ProjectSettings sync, ContentRegistry propagation, fight rewards, starting gold, and encounter HP/damage scaling from `RunState` into `BalanceManager`. `RunState` remains the public facade for prototype balance APIs and still owns active encounter assignment/scaling timing. (source: `scripts/core/balance_manager.gd`, `scripts/core/run_state.gd`)
- Updated [[architecture]] and [[file-map]] for the new helper ownership and recorded validation in `docs/test_plan.md` and `docs/architecture_review_tasks.md`. (source: `wiki/architecture.md`, `wiki/file-map.md`, `docs/test_plan.md`, `docs/architecture_review_tasks.md`)
- Validation: worker patch accepted at `8.7/10`; `git diff --check`, Godot MCP `view_script`, focused balance parity/propagation/scaling probes, `play_scene main`, `stop_running_scene`, and `get_godot_errors` passed with no runtime errors. (source: `docs/architecture_review_tasks.md`, `docs/test_plan.md`)

## 2026-05-07 - AR-26 RunState signal facade
...
## 2026-05-07 - CombatView rendering ownership extraction

- Moved combat HUD snapshot application, top/enemy/timer/player/debug rendering, timer display sync, enemy intent bubbles, enemy block preview rendering, hover signal emission, visual chrome, backdrop/scrim helpers, zone guides, loadout rail layout calls, and `CombatLayoutPresenter` application into `CombatView`. `CombatController` keeps combat orchestration, input phase authority, resolver kickoff, replay/VFX timing decisions, audio hooks, debug callbacks, RunState/FlowTrace routing, `/skip`, and outcome routing. (source: `scripts/combat/combat_view.gd`, `scripts/combat/combat_controller.gd`)
- Review gate accepted the worker patch at `8.2/10`. Validation passed `git diff --check`, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope probe, focused board local-coordinate/touch probe, `play_scene main`, `stop_running_scene`, and final `get_godot_errors` with no session errors. A follow-up parser-crash fix removed invalid no-op compatibility statements and explicitly typed the view layout result; Godot MCP then reached `combat_first_usable_frame` and resolve trace output. User manual QA passed after the fix. (source: `docs/test_plan.md`, `docs/architecture_review_tasks.md`)

## 2026-05-07 - CombatController view-node compatibility cleanup

- Removed leftover controller reach-ins to private `CombatView` helper nodes, dead view-rendering compatibility wrappers, unused controller-owned intent/block preview declarations, and the generic `CombatView.node(...)` accessor. `CombatView.refresh_character_portraits(...)` now owns portrait/backdrop refresh while `CombatController` keeps orchestration boundaries for board binding, presenters, outcome overlay, debug console, action buttons, replay timing, and RunState/FlowTrace routing. (source: `scripts/combat/combat_controller.gd`, `scripts/combat/combat_view.gd`)
- Review gate accepted the worker patch at `8.4/10`. Validation passed static searches, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope probe, focused board local-coordinate/touch probe, `play_scene current`, resolve trace through match flash/clear/gravity/refill/final commit, `stop_running_scene`, and final `get_godot_errors` with no current crash; stale editor reload warnings remain in the session. User manual QA passed on 2026-05-07 for real drag, resolve animation feel, enemy intent/block preview hover rendering, victory/defeat/boss reward routes, and debug commands. (source: `docs/test_plan.md`, `docs/architecture_review_tasks.md`)

## 2026-05-07 - CombatModel scene-local state ownership

- Moved combat scene-local mutable bookkeeping from `CombatController` into `CombatModel`: input phase, external lock reason, `combat_speed`, FlowTrace route id, pending next-scene path, outcome transition lock, hovered board orb id, staged HUD replay values, combat mastery preview totals/token, and resolve trace active/origin/pass state. `CombatController` remains the orchestration owner for domain collaborators, RunState/FlowTrace side effects, board binding, debug callbacks, replay/VFX timing, and route actions. (source: `scripts/combat/combat_model.gd`, `scripts/combat/combat_controller.gd`)
- Review gate accepted the worker patch at `8.1/10`. Validation passed static migration searches, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope probe, focused board local-coordinate/touch probe, and `play_scene current` after a pre-bind model guard fixed an initial `_enter_tree()` nil-model crash. Final `get_godot_errors` had no current runtime crash, but still reported existing unused class-variable and enum reload warnings; manual QA remains pending for this exact state-ownership pass. (source: `docs/test_plan.md`, `docs/architecture_review_tasks.md`)

## 2026-05-07 - Combat layout presenter ownership cleanup

- Removed redundant `CombatLayoutPresenter` ownership from `CombatController`: the controller no longer preloads, stores, binds, or mirrors layout presenter results. `CombatView` is the sole owner of layout presenter construction/binding/application, while `_apply_combat_layout()` remains a thin controller orchestration call into the view. (source: `scripts/combat/combat_controller.gd`, `scripts/combat/combat_view.gd`)
- Review gate accepted the worker patch at `8.3/10`. Validation passed static removal searches, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope probe, focused board local-coordinate/touch probe, and combat `play_scene current` smoke through `combat_first_usable_frame`; final `get_godot_errors` had no current runtime crash, with existing reload-warning noise still present in the editor session. (source: `docs/test_plan.md`, `docs/architecture_review_tasks.md`)

## 2026-05-07 - Combat VFX target lookup cleanup

- Moved enemy/player VFX target geometry lookup and VFX presenter binding-node packaging from `CombatController` into `CombatView`. The controller still owns replay/VFX timing decisions and keeps compatibility fallbacks, while `CombatVfxPresenter` continues to own VFX drawing mechanics. (source: `scripts/combat/combat_view.gd`, `scripts/combat/combat_controller.gd`, `scripts/combat/combat_vfx_presenter.gd`)
- Review gate accepted the worker patch at `8.4/10`. Validation passed static ownership searches, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope probe, focused board local-coordinate/touch probe, and combat `play_scene current` smoke with resolve trace through match flash, clear, combo ticks, gravity/refill, animation drain, final board commit, and turn replay FlowTrace; final `get_godot_errors` had no current runtime crash, with existing reload-warning noise still present in the editor session. User manual QA passed on 2026-05-07. (source: `docs/test_plan.md`, `docs/architecture_review_tasks.md`)

## 2026-05-07 - Combat SOLID HUD/debug extraction

- Moved combat HUD/intent presentation-data ownership further out of `CombatController`: `CombatHudPresenter` now owns HUD snapshot assembly, timer-state derivation, top-step labels, primary intent badge data, enemy intent preview rows, and intent damage preview dictionaries. `CombatController` now gathers domain/staged values and delegates presentation data plus rendering. (source: `scripts/combat/combat_hud_presenter.gd`, `scripts/combat/combat_controller.gd`, `scripts/combat/combat_view.gd`)
- Added `CombatDebugCommandAdapter` as the debug command effect boundary. `CombatDebugConsole` keeps parsing/help/log rendering, while the adapter owns command effects for state snapshots, `/skip`, board seed/reroll/print, gold/mastery/equipment/consumable/relic commands, and debug win/lose behind narrow controller-injected orchestration callbacks. Outcome flow extraction remains deferred because route semantics, boss reward state, FlowTrace, rollback, and pending-scene handling are still tightly coupled. (source: `scripts/combat/combat_debug_command_adapter.gd`, `scripts/combat/combat_debug_console.gd`, `scripts/combat/combat_controller.gd`)
- Updated [[file-map]], [[features]], and `docs/test_plan.md` for the new ownership boundary. Review gate accepted this slice at `8.2/10`; validation passed static ownership searches, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope probe, focused board local-coordinate/touch probe, and combat scene smoke through first usable frame. Manual route/debug QA remains pending for this exact slice. (source: `docs/test_plan.md`)

## 2026-05-07 - Combat MVC/SOLID model-view cleanup

- Expanded `CombatModel` as the scene-local state owner for staged HUD replay mutation helpers and combat mastery feedback release bookkeeping while keeping domain state, resolver math, RunState, and replay timing outside the model. (source: `scripts/combat/combat_model.gd`, `scripts/combat/combat_controller.gd`)
- Moved normal-path shared `PlayerLoadoutHud` rendering/control calls behind `CombatView` methods for player loadout render, outside-click focus handling, slot popover hiding, content lookup, mastery hover, and mastery feedback. Controller access remains for setup/signal binding and defensive compatibility fallbacks. (source: `scripts/combat/combat_view.gd`, `scripts/combat/combat_controller.gd`, `scripts/ui/player_loadout_hud.gd`)
- Added `CombatConsumableService` for combat-scoped consumable effect application through injected board conversion callbacks. `CombatController` still owns combat flow, consumable eligibility, status/audio/HUD consequences, route semantics, debug callbacks, and resolve/replay timing. Review gate accepted this slice at `8.3/10`; Godot MCP validation reached `combat_first_usable_frame`, preserved the AR-01 combat envelope baseline, preserved board-local mouse/touch drag probing, and confirmed the service conversion boundary. Manual QA remains pending for consumable use and mastery hover/replay visuals. (source: `scripts/combat/combat_consumable_service.gd`, `docs/test_plan.md`)

## 2026-05-07 - CombatController view compatibility surface removal

- Moved the remaining shared HUD node-map assembly, `PlayerLoadoutHud` binding handoff, visual-chrome HUD node packaging, VFX layer visibility, and enemy/player VFX target geometry fallback surface into `CombatView`. `CombatController` now calls view methods for the normal player HUD/control path and keeps setup/signal binding plus combat flow orchestration. No new folders, runtime files, presenters, services, models, views, or scene paths were added. (source: `scripts/combat/combat_view.gd`, `scripts/combat/combat_controller.gd`)
- Review gate accepted this cleanup at `8.4/10`. Godot MCP validation loaded the changed combat scripts and required collaborators, instantiated combat/main-menu/shop/run-summary scenes, preserved the AR-01 combat envelope baseline, preserved the board-local mouse/touch drag probe, and reached `combat_first_usable_frame` in `res://scenes/combat.tscn`; user manual QA passed after the cleanup. (source: `docs/test_plan.md`, `scripts/debug/ar01_combat_result_probe.gd`, `scripts/board/board_controller.gd`)

## 2026-05-07 - CombatController render-only mirror field cleanup

- Removed render-only, unused controller mirror declarations for timer, enemy, chrome, layout, divider, corner, and retired HUD visual nodes now owned by `CombatView`. The controller still receives the root node dictionary, but only retained orchestration boundary fields continue to mirror into `CombatController`. No new files or folder structure changes were made. (source: `scripts/combat/combat_controller.gd`, `scripts/combat/combat_view.gd`)
- Review gate accepted this deletion-only cleanup at `8.6/10`. Validation passed static declaration searches, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope, focused board-local mouse/touch drag probe, and combat scene smoke through `combat_first_usable_frame`; user manual QA passed after the cleanup. The current error log no longer shows the large unused controller-field warning list, though the editor session still carries two existing enum reload messages. (source: `docs/test_plan.md`, `scripts/debug/ar01_combat_result_probe.gd`, `scripts/board/board_controller.gd`)

## 2026-05-07 - CombatView status/debug/outcome boundary cleanup

- Moved status-label, turn-summary, debug overlay/console-node binding, and outcome overlay node/button/body UI access behind `CombatView` methods. `CombatController` still owns combat flow, debug command callbacks, RunState/FlowTrace routing, boss reward claim/skip, input phase, resolver kickoff, VFX timing, and replay order. No new folders, runtime files, or scene paths were added. (source: `scripts/combat/combat_view.gd`, `scripts/combat/combat_controller.gd`)
- Review gate accepted this cleanup at `8.5/10`. Validation passed static ownership searches, `git diff --check`, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope, focused board-local mouse/touch drag probe, and combat scene smoke through `combat_first_usable_frame`; final errors showed no current runtime crash and only the existing two enum reload diagnostics. User manual QA passed on 2026-05-07. (source: `docs/test_plan.md`, `scripts/debug/ar01_combat_result_probe.gd`, `scripts/board/board_controller.gd`)

## 2026-05-07 - CombatView bootstrap boundary cleanup

- Removed final transitional view-bootstrap scene-node mirrors from `CombatController`: background, layout root, board panel, title label, and hint label. `CombatView` now owns background bootstrap, top-bar bootstrap text, shared HUD popover parent resolution, and resolve-presenter node packaging for the view-owned board panel. `CombatController` keeps combat flow, board orchestration, resolve presenter creation/speed, callbacks, RunState/FlowTrace routing, debug/outcome flow, VFX timing, and replay order. (source: `scripts/combat/combat_controller.gd`, `scripts/combat/combat_view.gd`)
- Review gate accepted this cleanup at `8.6/10`. Validation passed static ownership searches, `git diff --check`, Godot MCP script loads, scene instantiate checks, retained AR-01 combat envelope, focused board-local mouse/touch drag probe, and combat scene smoke through `combat_first_usable_frame`; final errors showed no current runtime crash and only the existing two enum reload diagnostics. User manual QA passed on 2026-05-07. (source: `docs/test_plan.md`, `scripts/debug/ar01_combat_result_probe.gd`, `scripts/board/board_controller.gd`)

## 2026-05-07 - CombatController wrapper/delegate cleanup
...
- Completed a multi-agent internal QA documentation pass for Wave 1 assets after human QA was skipped by instruction. Treasure chest icons, run-summary background, and itch key art moved to internal QA passed/review-pending states with legal/license and integration gates still blocked; result-screen UI pack remains cleanup-required because it is a concept sheet rather than separated runtime UI chrome. (source: `assets/qa/reports/wave_01_public_demo_core_status.md`, `assets/qa/reports/qa_status_manifest.json`)

## 2026-05-07 - Assetgen bulk-generation-first workflow mode

- Updated `resources/assetgen.workflow.md` and [[assetgen-workflow]] to document `bulk_generation_first` execution: generate all image-capable candidates from the `assets.json` wave first, defer QA/cleanup to one later batch, and keep legal/license review, final approval, and runtime integration blocked until deferred QA and follow-up review are complete. Clarified tooling boundaries: `imagegen` for image candidates, `game-studio:sprite-pipeline` only after image candidates for sprite/animation normalization, and audio/screenshot/video assets as non-image tooling or spec-placeholder tracks until explicitly requested. (source: `resources/assetgen.workflow.md`, `wiki/assetgen-workflow.md`)
- Added `assets/generated/metadata/bulk_generation_plan.json` as a machine-readable plan tied to the 53-record metadata manifest, including deferred-QA policy and per-asset routing lists for imagegen, sprite-pipeline follow-up, and external tooling/spec placeholders. (source: `assets/generated/metadata/assets.json`, `assets/generated/metadata/bulk_generation_plan.json`, `assets.json`)

## 2026-05-08 - Assetgen bulk metadata sync

- Synced aggregate assetgen metadata after the bulk-generation candidate batch: 37 of 38 planned image-generation records now have candidate PNGs and per-record metadata; `enemy_animation_set` remains the only planned image-generation record without a candidate file. Updated the aggregate generated metadata manifest, bulk-generation progress block, QA status manifest, approval status manifest, and [[assetgen-workflow]] status note. QA, human/legal review, final approval, and runtime integration remain blocked by the `bulk_generation_first` gate. (source: `assets/generated/metadata/assets.json`, `assets/generated/metadata/bulk_generation_plan.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`, `wiki/assetgen-workflow.md`)

## 2026-05-08 - Assetgen imagegen invocation clarification

- Clarified `resources/assetgen.workflow.md` and [[assetgen-workflow]] to prevent agents from treating `image_gen` or `imagegen` as shell commands. Generated raster assets must use the Codex `imagegen` skill and the Codex-provided in-chat image generation tool from the agent tool interface; CLI fallback is only allowed when explicitly requested by the human and documented by the skill. (source: `resources/assetgen.workflow.md`, `wiki/assetgen-workflow.md`)

## 2026-05-08 - Assetgen bulk image generation complete

- Generated the final planned bulk image candidate, `enemy_animation_set`, as a review-only enemy animation source-frame sheet for later sprite-pipeline follow-up. Synced the per-record metadata and aggregate generated/QA/approval manifests so the bulk plan now reports 38 of 38 image-generation records complete with 0 remaining. QA, legal/license review, human review, final approval, sprite-pipeline normalization, and runtime integration remain blocked until the deferred batch gates reopen. (source: `assets/generated/candidates/bulk-generation-first/enemy_animation_set_candidate_01.png`, `assets/generated/metadata/records/enemy_animation_set.json`, `assets/generated/metadata/assets.json`, `assets/generated/metadata/bulk_generation_plan.json`, `wiki/assetgen-workflow.md`)
## 2026-05-08 - Assetgen deferred transparency QA pass

- Recorded a consolidated deferred transparency QA pass for generated candidate PNGs in `assets/qa/reports/bulk_transparency_qa.md`, grounded in `assets/qa/transparency_tests/transparency_probe_current.json`. Coverage summary: 38 generated candidate records, 21 alpha-sensitive asset records checked (24 sensitive PNG files), and only one file with real alpha (`assets/cleanup/treasure_chest_icons_candidate_03_alpha_cleanup.png`). (source: `assets/qa/reports/bulk_transparency_qa.md`, `assets/qa/transparency_tests/transparency_probe_current.json`)
- Updated alpha-sensitive metadata outcomes so UI/VFX/icon-sheet records plus `result_screen_ui_pack` and sprite-source sheets (`hero_animation_set`, `enemy_animation_set`) are marked `cleanup_required` with explicit alpha notes/flags. Sprite-source sheets remain deferred to sprite-pipeline normalization/cleanup. (source: `assets/generated/metadata/records/board_orb_clear_vfx.json`, `assets/generated/metadata/records/result_screen_ui_pack.json`, `assets/generated/metadata/records/hero_animation_set.json`, `assets/generated/metadata/records/enemy_animation_set.json`)
- Synced aggregate and gate manifests from per-record metadata: `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, and `assets/approved/status_manifest.json`. Cleanup-required assets now stay blocked in approval/integration manifests; opaque/non-alpha-sensitive generated candidates remain non-failed for alpha in this pass. Runtime integration remains blocked pending cleanup and follow-up QA/review gates. (source: `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`)
- Ran deterministic edge-connected alpha cleanup for eligible alpha-sensitive generated candidates. The pass created 21 alpha-fixed PNGs under `assets/cleanup/alpha_fixed/`, recorded full probe evidence in `assets/qa/transparency_tests/alpha_cleanup_probe_report.full.json`, and added `assets/qa/transparency_tests/alpha_cleanup_background_contact_sheet.png` for white/black/gray/checkerboard visual review. Nineteen records moved to `qa_pending_batch`; `hero_animation_set` and `enemy_animation_set` remain `cleanup_required` for sprite-pipeline normalization. (source: `assets/qa/transparency_tests/alpha_cleanup_probe.py`, `assets/qa/reports/bulk_transparency_qa.md`, `assets/generated/metadata/assets.json`)
- Added `assets/qa/reports/alpha_fixed_quality_control.md` as a follow-up quality-control pass on the actual alpha-fixed PNGs. The pass separates 12 files that can proceed into deferred QA from 9 files needing targeted visual review for possible glow, edge, or component-sheet damage; animation source sheets remain deferred to sprite-pipeline extraction. (source: `assets/qa/reports/alpha_fixed_quality_control.md`, `assets/qa/transparency_tests/alpha_cleanup_background_contact_sheet.png`)

## 2026-05-08 - Assetgen deterministic alpha cleanup metadata sync

- Recorded deterministic alpha-cleanup outputs from `assets/qa/transparency_tests/alpha_cleanup_probe_report.full.json`: 21 generated files now exist under `assets/cleanup/alpha_fixed/`, each with no report warnings and real-alpha evidence. (source: `assets/qa/transparency_tests/alpha_cleanup_probe_report.full.json`, `assets/cleanup/alpha_fixed/`)
- Updated 19 per-record metadata files to attach alpha-fixed candidates and move those records to `qa_pending_batch` (not approved), while keeping `hero_animation_set` and `enemy_animation_set` as `cleanup_required` deferred to sprite-pipeline cleanup. Runtime integration remains blocked. (source: `assets/generated/metadata/records/board_orb_clear_vfx.json`, `assets/generated/metadata/records/treasure_chest_icons.json`, `assets/generated/metadata/records/hero_animation_set.json`, `assets/generated/metadata/records/enemy_animation_set.json`)
- Regenerated aggregate and gate manifests from per-record metadata and synced the bulk transparency QA report and [[assetgen-workflow]] status note to the new alpha-fixed pending-batch state. (source: `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`, `assets/qa/reports/bulk_transparency_qa.md`, `wiki/assetgen-workflow.md`)

## 2026-05-08 - Assetgen true-alpha regeneration blocker

- Ran a multi-agent regeneration pass for `board_orb_clear_vfx`, `consumable_icons`, `enemy_attack_vfx`, and `mastery_vfx` using the required built-in `imagegen` workflow with explicit true-alpha constraints. The regenerated `candidate_02` PNGs were fully opaque (`min_alpha=255`, `max_alpha=255`) and visually contained baked checkerboard/false transparency, so they were rejected instead of promoted. (source: `assets/rejected/alpha_failed/`, `assets/qa/reports/bulk_transparency_qa.md`)
- Moved the rejected alpha-fixed and failed `candidate_02` files for those four records to `assets/rejected/alpha_failed/`, removed them from active candidate paths, and synced per-record metadata plus aggregate QA/approval manifests so review and integration remain blocked until a generation path can output true alpha during generation. (source: `assets/generated/metadata/records/board_orb_clear_vfx.json`, `assets/generated/metadata/records/consumable_icons.json`, `assets/generated/metadata/records/enemy_attack_vfx.json`, `assets/generated/metadata/records/mastery_vfx.json`, `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`)

## 2026-05-08 - Assetgen chroma-key fallback technique

- Documented a governed chroma-key transparency fallback for cases where true-alpha generation is unavailable or repeatedly produces fake transparency. The technique requires an asset-specific flat synthetic key color, forbids checkerboards/gradients/shadows/key-color use inside the asset, runs deterministic keyed-alpha extraction, and promotes only the extracted RGBA PNG after probe/visual checks; the opaque keyed source remains trace evidence. (source: `resources/assetgen.workflow.md`, `wiki/assetgen-workflow.md`)
- Generated `candidate_03_chroma_key` keyed sources for `board_orb_clear_vfx`, `consumable_icons`, `enemy_attack_vfx`, and `mastery_vfx`. Exact-key extraction was unreliable because built-in imagegen softened the key backgrounds, so `assets/qa/transparency_tests/adaptive_border_extract.py` sampled connected border backgrounds and produced real-alpha RGBA outputs under `assets/cleanup/chroma_keyed_adaptive/` with empty probe warnings. Metadata and aggregate manifests now stage those extracted outputs for deferred QA while keeping approval and integration blocked. (source: `assets/generated/candidates/bulk-generation-first/board_orb_clear_vfx_candidate_03_chroma_key.png`, `assets/cleanup/chroma_keyed_adaptive/`, `assets/qa/transparency_tests/adaptive_border_extract_report.candidate_03.json`, `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`)
- Regenerated the full 21-record alpha-sensitive set with the chroma-key fallback as `candidate_04_chroma_key` sources and extracted real-alpha RGBA outputs under `assets/cleanup/chroma_keyed_adaptive_all/`. The full extraction report records `alpha_minmax: [0, 255]` and empty warnings for all 21 outputs, with a contact sheet for white/black/gray/checkerboard review. Synced per-record metadata, aggregate metadata, QA status, approval status, and [[assetgen-workflow]] so all 21 alpha-sensitive records are `qa_pending_batch` while approval and integration remain blocked. (source: `assets/cleanup/chroma_keyed_adaptive_all/`, `assets/qa/transparency_tests/adaptive_border_extract_report.candidate_04.full.json`, `assets/qa/transparency_tests/adaptive_candidate_04_contact_sheet.png`, `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`, `wiki/assetgen-workflow.md`)
- Added a follow-up visual quality-control report for the 21 adaptive chroma-key outputs. The report splits the real-alpha candidates into 12 ordinary deferred-QA candidates, 7 targeted component/visual review candidates, and 2 sprite-pipeline normalization candidates; none are approved or integrated. (source: `assets/qa/reports/adaptive_chroma_key_quality_control.md`, `assets/qa/transparency_tests/adaptive_candidate_04_contact_sheet.png`)
- Recorded the human review decision for the adaptive chroma-key outputs: `main_menu_ui_pack`, `shared_hud_chrome`, and `tutorial_onboarding_ui_pack` need another pass; `hero_animation_set` and `enemy_animation_set` can proceed to sprite-pipeline normalization; all other adaptive outputs are fine to proceed from deferred QA toward legal/final review. Updated metadata, manifests, the QC report, bulk transparency report, and the asset review page categories. (source: `assets/qa/reports/adaptive_chroma_key_quality_control.md`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`, `assets/index.html`)
- Completed the follow-up cleanup and sprite-pipeline pass: candidate_05 UI retry outputs were generated/extracted for `main_menu_ui_pack`, `shared_hud_chrome`, and `tutorial_onboarding_ui_pack`; sprite-pipeline normalization produced raw frames, 512x512 normalized frames, preview strips, and contact sheets for `hero_animation_set` and `enemy_animation_set`. Metadata/manifests now put those five records back in `review_pending` while approval and integration remain blocked. (source: `assets/cleanup/chroma_keyed_adaptive_ui_retry/`, `assets/qa/transparency_tests/adaptive_border_extract_report.candidate_05.ui_retry.json`, `assets/sprite_pipeline/candidate_04/`, `assets/qa/reports/sprite_pipeline_candidate_04.md`, `assets/index.html`)
- Recorded the latest human review: `main_menu_ui_pack` and `shared_hud_chrome` candidate_05 retries are good; `tutorial_onboarding_ui_pack` still has one magenta artifact; `hero_animation_set` and `enemy_animation_set` sprite previews are bad. Updated metadata/manifests and reordered `assets/index.html` so only the remaining three problem assets are highlighted for attention. (source: `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`, `assets/index.html`)
- Regenerated the remaining three blocked assets as candidate_06 using the chroma-key fallback, including applying the fallback before sprite normalization for `hero_animation_set` and `enemy_animation_set`. Adaptive extraction reported no warnings, sprite-pipeline candidate_06 previews were generated, and `assets/index.html` now points the priority review queue at the new candidate_06 outputs. (source: `assets/generated/candidates/bulk-generation-first/tutorial_onboarding_ui_pack_candidate_06_chroma_key.png`, `assets/cleanup/chroma_keyed_adaptive_candidate_06/`, `assets/qa/transparency_tests/adaptive_border_extract_report.candidate_06.remaining.json`, `assets/sprite_pipeline/candidate_06/`, `assets/index.html`)
- Continued the animation rework after candidate_06: `hero_animation_set` now uses candidate_07 from a magenta-key source, while `enemy_animation_set` now uses candidate_08 from a cyan-key source with component-based slicing from the extracted alpha sheet. Updated metadata/manifests and `assets/index.html` so the latest review queue uses tutorial candidate_06, hero candidate_07, and enemy candidate_08. (source: `assets/generated/candidates/bulk-generation-first/hero_animation_set_candidate_07_chroma_key.png`, `assets/generated/candidates/bulk-generation-first/enemy_animation_set_candidate_08_chroma_key.png`, `assets/cleanup/chroma_keyed_adaptive_candidate_07/`, `assets/cleanup/chroma_keyed_adaptive_candidate_08/`, `assets/sprite_pipeline/candidate_07/`, `assets/sprite_pipeline/candidate_08_component/`, `assets/index.html`)
- Corrected the latest review artifacts after browser inspection showed remaining visible key color and checker-composited animation previews. `tutorial_onboarding_ui_pack` now uses soft global key extraction to remove disconnected magenta inside the ring, and the animation previews now use transparent soft-eroded RGBA strips instead of RGB checker composites. (source: `assets/cleanup/chroma_keyed_soft_candidate_06/`, `assets/cleanup/chroma_keyed_soft_candidate_07_eroded/`, `assets/cleanup/chroma_keyed_soft_candidate_08_eroded/`, `assets/sprite_pipeline/candidate_07_soft_eroded_component/`, `assets/sprite_pipeline/candidate_08_soft_eroded_component/`, `assets/index.html`)
- Recorded the human acceptance of the final three problem assets: `tutorial_onboarding_ui_pack`, `hero_animation_set`, and `enemy_animation_set` are now visually accepted as good and moved out of the attention queue. Metadata/manifests keep all three blocked from integration pending legal/license review and final approval. (source: `assets/generated/metadata/records/tutorial_onboarding_ui_pack.json`, `assets/generated/metadata/records/hero_animation_set.json`, `assets/generated/metadata/records/enemy_animation_set.json`, `assets/index.html`)
- Recorded the human approval gate for the assetgen batch. Per-record metadata, aggregate metadata, QA status, and approval manifests now mark the assets as `approved` with `integration_status: approved_pending_integration`; runtime integration still needs a separate copy/import pass into Godot asset locations. (source: `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`)
- Started the runtime integration test with main menu only. `main_menu_background_candidate_01.png` is copied into `resources/art/assetgen/main_menu/`, and `resources/visual/first_pass_asset_map.json` now points both main-menu background keys at that runtime copy. The approved `main_menu_ui_pack` remains pending deterministic sheet slicing before it can replace the existing separate menu chrome textures. (source: `resources/art/assetgen/main_menu/main_menu_background_candidate_01.png`, `resources/visual/first_pass_asset_map.json`, `wiki/main-menu-assets.md`)
- Recorded human visual acceptance of the main-menu-only background integration test. `main_menu_background` now carries `integration_status: integrated_test_main_menu` in the per-record metadata, aggregate metadata, QA manifest, and approval manifest; this does not integrate the packed `main_menu_ui_pack`. (source: `assets/generated/metadata/records/main_menu_background.json`, `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`)

## 2026-05-08 - Main-menu assetgen candidate_05 UI slicing integration

- Added deterministic main-menu UI extraction script `tools/asset_tools/extract_assetgen_main_menu_ui.py` that slices `assets/cleanup/chroma_keyed_adaptive_ui_retry/main_menu_ui_pack_candidate_05_adaptive_alpha.png` by alpha connected components into runtime pieces under `resources/art/assetgen/main_menu/`, plus a JSON extract report and preview contact sheet. (source: `tools/asset_tools/extract_assetgen_main_menu_ui.py`, `resources/art/assetgen/main_menu/main_menu_ui_pack_candidate_05_extract_report.json`, `resources/art/assetgen/main_menu/preview/main_menu_ui_pack_candidate_05_components_preview.png`)
- Updated `resources/visual/first_pass_asset_map.json` so main-menu chrome/icon mappings (`outer_border`, `button_primary`, `button_secondary`, `stats_panel`, and `menu.menu_icons.*`) now point to candidate_05 runtime slices, while `menu.logo` and the already accepted assetgen background mapping remain unchanged. (source: `resources/visual/first_pass_asset_map.json`, `resources/art/assetgen/main_menu/`)
- Synced [[main-menu-assets]] to document the deterministic slicing workflow and current candidate_05 runtime wiring status. (source: `wiki/main-menu-assets.md`)
- Marked `main_menu_ui_pack` as `integrated_test_main_menu` in the per-record metadata, aggregate metadata, QA manifest, and approval manifest after wiring candidate_05 slices into the main-menu asset map. This status covers only the main-menu test integration, not other screens. (source: `assets/generated/metadata/records/main_menu_ui_pack.json`, `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`)

## 2026-05-08 - Main-menu assetgen candidate_05 spill cleanup, semantic icons, and layout pass

- Updated `tools/asset_tools/extract_assetgen_main_menu_ui.py` so candidate_05 extraction now includes deterministic edge-connected magenta spill cleanup on each crop, semantic icon compositing (`*_semantic.png`) by overlaying existing first-pass menu glyphs into candidate_05 medallion frames, and an optional frame-only slab variant (`main_menu_stats_panel_candidate_05_frame_only.png`). The script report now records cleanup counts plus semantic/optional outputs. (source: `tools/asset_tools/extract_assetgen_main_menu_ui.py`, `resources/art/assetgen/main_menu/main_menu_ui_pack_candidate_05_extract_report.json`, `resources/art/assetgen/main_menu/`)
- Updated `resources/visual/first_pass_asset_map.json` main-menu icon paths only so `menu.menu_icons.*` uses semantic candidate_05 composites while keeping the candidate_05 slab (`menu.stats_panel`) wired by default. (source: `resources/visual/first_pass_asset_map.json`)
- Rebalanced main-menu runtime layout percentages and footer/version sizing for the candidate_05 proportions to prevent top/logo overlap and reduce bottom cramping without changing menu behavior. (source: `scripts/main_menu/main_menu_view.gd`)
- Synced [[main-menu-assets]] with the deterministic cleanup/composite workflow and optional frame-only output status. (source: `wiki/main-menu-assets.md`)

## 2026-05-08 - Main-menu game title logo alpha integration

- Added `tools/asset_tools/prepare_main_menu_logo_alpha.py` to deterministically convert `assets/generated/candidates/bulk-generation-first/game_title_logo_candidate_01.png` into a runtime-ready true-alpha logo by removing edge-connected dark background pixels from border-seeded flood fill; the script emits `resources/art/assetgen/main_menu/game_title_logo_candidate_01_alpha.png` and a JSON cleanup report for repeatability. (source: `tools/asset_tools/prepare_main_menu_logo_alpha.py`, `resources/art/assetgen/main_menu/game_title_logo_candidate_01_alpha.png`, `resources/art/assetgen/main_menu/game_title_logo_candidate_01_alpha_report.json`)
- Updated the main-menu runtime map and layout for this test integration: `menu.logo` now maps to the cleaned runtime logo asset and the logo rect is larger/lower (`x=0.03`, `y=0.065`, `w=0.94`, `h=0.16`) to address prior small/high placement feedback. (source: `resources/visual/first_pass_asset_map.json`, `scripts/main_menu/main_menu_view.gd`)
- Marked `game_title_logo` as `integration_status: integrated_test_main_menu` in per-record and aggregate metadata manifests for this scoped menu integration test. (source: `assets/generated/metadata/records/game_title_logo.json`, `assets/generated/metadata/assets.json`, `assets/qa/reports/qa_status_manifest.json`, `assets/approved/status_manifest.json`)
- Recorded human visual acceptance of the main-menu runtime wiring. `main_menu_background`, `main_menu_ui_pack`, and `game_title_logo` now carry `integration_status: integrated_main_menu` in per-record metadata, aggregate metadata, QA manifest, and approval manifest. (source: `assets/generated/metadata/records/main_menu_background.json`, `assets/generated/metadata/records/main_menu_ui_pack.json`, `assets/generated/metadata/records/game_title_logo.json`, `resources/visual/first_pass_asset_map.json`)

## 2026-05-08 - Main-menu focused layout polish pass

- Updated main-menu runtime layout/view behavior to hide `GenerateLogToggle` from the main UI while preserving the existing log-export setting pipeline and signal path through `RunState`.
- Hid the visible `ElementRow` (elemental icons/labels) from the main-menu surface without deleting scene nodes or removing mapped mastery assets/content.
- Centered the primary menu stack inside the slab by using centered `VBoxContainer` alignment and updated menu column rects; primary visible actions remain `Start Run`, `Continue`, `Collection`, `Settings`, and `Quit`.
- Increased and lowered the runtime logo region relative to the prior accepted main-menu layout so the wordmark reads substantially larger without changing source art assets. (source: `scripts/main_menu/main_menu_view.gd`, `scripts/main_menu/main_menu_controller.gd`, `wiki/main-menu-assets.md`)

## 2026-05-08 - Main-menu button readability styling pass

- Updated `scripts/main_menu/main_menu_view.gd` so main-menu action buttons, footer buttons, and profile overlay action buttons now use a restrained shared dark `StyleBoxFlat` treatment instead of texture-plate styling from `menu.button_primary` and `menu.button_secondary`. This removes decorative/ornament-heavy runtime button chrome while preserving button wiring and behavior.
- Kept `Start Run` slightly emphasized through cooler/lighter border and text colors without using parchment/gold fill, and preserved disabled-state style paths for menu/footer/profile actions. (source: `scripts/main_menu/main_menu_view.gd`, `wiki/main-menu-assets.md`)
