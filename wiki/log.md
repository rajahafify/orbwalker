...
- Updated [[features]] for CFR-08 enemy intent HP preview: before the player moves, combat now computes projected incoming attack damage from the current enemy intent and visible player HP/armor, then the shared HUD shows a red-to-empty-to-red blinking HP danger segment for unblocked or partially blocked HP loss and an armor-risk pulse for fully blocked attacks. The warning fades over an empty HP backing so the off phase does not reveal the normal red HP fill. The combat controller also computes an enemy intent preview dictionary with an `entries` array for future multi-intent display, currently used for a blinking silver enemy block/overshield preview above the enemy HP bar. Hovering either preview only scales/blinks the existing enemy intent display/status text while the segment remains hovered and does not change combat math, enemy intent values, resolver behavior, RunState routing, enemy attack resolution, replay timing, board order, or `combat_speed`. Godot MCP validation passed on 2026-05-04; manual visual QA remains useful for real enemy-intent cases. (source: `scripts/combat/combat_player_controller.gd`, `scripts/ui/player_loadout_hud.gd`, `docs/combat_feedback_revamp_tasks.md`)
