...
- [x] Create a minimal boot scene.
  - Deliverable: `res://scenes/main_menu.tscn` or equivalent main scene configured in `project.godot`.
...
- Milestone 9 graphical asset integration pass (combat + shop) is now wired through a dedicated visual registry and player-facing scenes:
  ...
  - Added player-facing scene routes:
	- Combat: `res://scenes/combat.tscn` (controller: `res://scripts/scenes/combat.gd`)
	- Shop: `res://scenes/shop.tscn` (controller: `res://scripts/scenes/shop.gd`)
...
- Reference player-panel revamp added for `res://scenes/combat.tscn`:
...
- [x] AR-10 Combat controller god-object refactor. Extracted `CombatDebugConsole` and `CombatTurnLogPresenter` while keeping privileged gameplay callbacks, `/skip`, routing, input, layout, VFX timing, and combat state in the controller.
- [x] AR-11 Combat layout presenter extraction. Extracted combat scene geometry and responsive layout ownership into `CombatLayoutPresenter` while preserving portrait/tall viewport behavior and shared HUD layout overrides.
- [x] AR-12 Combat VFX presenter extraction. Extracted transient combat VFX drawing mechanics into `CombatVfxPresenter` while keeping replay order, combat math, mastery feedback semantics, routing, input, layout, audio, debug callbacks, and `/skip` in the controller.
- [x] AR-13 Board controller extraction. Extracted board-local drag/pointer bookkeeping into `BoardController` while preserving `BoardView.gui_input` local-coordinate behavior, current drag rules, swap feel, resolve kickoff, combat math, timing, routing, layout, VFX, audio callback policy, debug callbacks, and `/skip` ownership in the combat controller.
...
- [x] AR-16 Combat HUD sync boundary review. Extracted side-effect-free combat HUD snapshot construction into `CombatHudPresenter` while keeping scene-specific label/bar/node application, `PlayerLoadoutHud` payload dispatch, loadout rail layout refresh, placeholder fallback assignment, combat-only enemy/timer/status behavior, shop HUD behavior, mastery feedback/card rendering, combat math, route transitions, and `/skip` ownership unchanged.
